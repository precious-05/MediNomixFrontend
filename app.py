...
# ==================== CONFIGURATION ====================
# Get backend URL from environment variable for production, fallback to local for development
BACKEND_URL = os.environ.get("BACKEND_URL", "http://localhost:8000")

# Dynamically create WebSocket URL based on backend URL
WS_URL = BACKEND_URL.replace("http://", "ws://").replace("https://", "wss://") + "/ws/dashboard"
...
if 'backend_health' not in st.session_state:
    st.session_state.backend_health = {"status": "unknown", "last_check": None}
...
def check_backend_health():
    ...
    try:
        # Check if we should skip health check (recent check within 30 seconds)
        if st.session_state.backend_health.get("last_check"):
            last_check = st.session_state.backend_health["last_check"]
...
def safe_api_call(func, *args, **kwargs):
    ...
    try:
        ...
        render_alert_card("Cannot connect to backend server. Please ensure the backend is running.", "danger")
        return None
    except requests.exceptions.Timeout:
        render_alert_card("Request timeout. The backend server might be busy or unreachable.", "warning")
        return None
    except Exception as e:
        render_alert_card(f"Error: {str(e)[:100]}", "danger")
        return None

# ================================
# REAL-TIME WEBSOCKET MANAGER (OPTIMIZED)
# ================================
...
class RealTimeWebSocketManager:
    ...
    def start_connection(self):
        if self.retry_count >= self.max_retries:
            render_alert_card("Maximum WebSocket connection retries reached. Please refresh the page.", "warning")
            return
            
        try:
            # Check if backend is healthy before attempting WebSocket
            health = check_backend_health()
            if health["status"] != "healthy":
                render_alert_card("Backend not available. Cannot establish WebSocket connection.", "warning")
                return
    ...
    def _on_open(self, ws):
        ...
        st.session_state.realtime_metrics = {}  # Clear old data
    ...
    def _on_error(self, ws, error):
        self.connected = False
        st.session_state.websocket_connected = False
        if "failed to connect" in str(error).lower():
            render_alert_card("WebSocket connection failed. Real-time features unavailable.", "warning", "Connection Failed")
        else:
...
def search_drug(drug_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            render_alert_card(f"Backend returned error: {response.status_code}", "warning")
            return None
    except Exception as e:
        render_alert_card(f"Error searching drug: {str(e)[:100]}", "danger")
        return None
...
def load_examples():
    ...
    try:
        ...
        if response.status_code == 200:
            render_alert_card("Examples loaded successfully!", "success")
            return True
        else:
            render_alert_card(f"Failed to load examples: {response.status_code}", "warning")
            return False
    except Exception as e:
        render_alert_card(f"Error loading examples: {str(e)[:100]}", "danger")
        return False

def load_dashboard_data():
    """Load dashboard analytics data with fallbacks"""
    try:
        dashboard_data = {}
        ...
        try:
            ...
            if metrics_response.status_code == 200:
                dashboard_data['metrics'] = metrics_response.json()
            else:
                dashboard_data['metrics'] = get_fallback_metrics()
        except:
            dashboard_data['metrics'] = get_fallback_metrics()
        ...
        try:
            ...
            if risks_response.status_code == 200:
                dashboard_data['top_risks'] = risks_response.json()
        except:
            dashboard_data['top_risks'] = []
        
        # Load risk breakdown
        try:
            breakdown_response = requests.get(f"{BACKEND_URL}/api/risk-breakdown", timeout=10)
            if breakdown_response.status_code == 200:
                dashboard_data['breakdown'] = breakdown_response.json()
        except:
            dashboard_data['breakdown'] = get_fallback_breakdown()
        
        # Load heatmap data
        try:
            heatmap_response = requests.get(f"{BACKEND_URL}/api/heatmap?limit=15", timeout=10)
            if heatmap_response.status_code == 200:
                dashboard_data['heatmap'] = heatmap_response.json()
        except:
            dashboard_data['heatmap'] = get_fallback_heatmap()
            
        st.session_state.dashboard_data = dashboard_data
        return True
    except Exception as e:
        render_alert_card(f"Error loading dashboard: {str(e)[:100]}", "warning")
        # Use fallback data
        st.session_state.dashboard_data = {
            'metrics': get_fallback_metrics(),
            'top_risks': [],
            'breakdown': get_fallback_breakdown(),
            'heatmap': get_fallback_heatmap()
        }
        return False

def get_fallback_metrics():
    """Provide fallback metrics when backend is unavailable"""
    return {
        "total_drugs": 0,
        "total_analyses": 0,
        "high_risk_pairs": 0,
        "critical_risk_pairs": 0,
        "avg_risk_score": 0,
        "recent_searches": [],
        "system_status": "backend_unavailable",
        "last_updated": datetime.utcnow().isoformat(),
        "connected_clients": 0
    }

def get_fallback_breakdown():
    """Provide fallback breakdown data"""
    return [
...
def render_realtime_tab():
    ...
    with col1:
        if st.session_state.websocket_connected:
            ...
            render_alert_card("🔌 Real-time features require WebSocket connection. Click 'Connect' to enable.", "info", "Ready to Connect")
    ...
    # Auto-start WebSocket if not connected (only if backend is healthy)
    ...
    if metrics and st.session_state.websocket_connected:
        # Real-time KPI Cards
        col1, col2, col3, col4 = st.columns(4)
        ...
        <div class="glass-card" style="text-align: center; padding: 40px 20px;">
            ...
            <p style="color: {COLORS['text_secondary']}; max-width: 400px; margin: 0 auto; font-size: 14px; font-weight: 500;">
                {f"Click 'Connect' above to enable real-time features" if not st.session_state.websocket_connected else "Live updates will appear here once data is received."}
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if health["status"] == "healthy":
            ...
            if health["data"]:
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("💊 Drugs", health["data"].get('metrics', {}).get('drugs_in_database', 0))
                with col2:
                    st.metric("📊 Analyses", health["data"].get('metrics', {}).get('total_analyses', 0))
        elif health["status"] == "timeout":
            render_alert_card("Backend connection timeout. Server might be busy.", "warning", "⚠️ Connection Timeout")
        ...
        # Environment Info
        is_production = os.environ.get("BACKEND_URL", "").startswith("https://")
        st.markdown(f"""
        <div style="margin: 16px 0; padding: 12px; background: {COLORS['primary']}10; border-radius: 12px; border-left: 4px solid {COLORS['primary']};">
            <div style="font-size: 11px; font-weight: 600; color: {COLORS['text_secondary']}; text-transform: uppercase; margin-bottom: 4px;">Environment</div>
            <div style="font-size: 14px; font-weight: 700; color: {COLORS['text_primary']};">{'Production' if is_production else 'Development'}</div>
        </div>
        """, unsafe_allow_html=True)
...
if __name__ == "__main__":
    ...
    if health["status"] != "healthy":
        render_alert_card(
            f"Backend server connection issue: {health['message']}. Some features may be limited.",
            "warning",
            "⚠️ Backend Warning"
        )
